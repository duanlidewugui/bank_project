...
def new_user(name,password,site,names):
    ...
    while True:
        key_uesr = 0
        account_number = int(random()*90000000+10000000)
        #if len(names) == 0:
        #    break
...
#开始main
names= [
    [10000000,"舒识",123456,("中国","湖南","盈丰路","417"),0,"中国工商银行北京昌平支行"]
]
while True:
    key2 = 0
    inface()
...
#转账
    if key2 == 4:
        while True:
            number1 = int(input("请输入会收到钱的八位账号"))
            number2 = int(input("请输入您的八位账号"))
            password = int(input("请输入您的六位密码"))
